	...
	switch(User_Key_Scan())								//按键扫描，单击，双击或长按
		{
			...
			case Click:
				...
				else if(Mode == Lab_Lidar_Radar_Mode)		//Lab11-12雷达实验模式
				{
					Lab_Lidar_Radar_Reset();
				}
				else if(Mode > Lab_Lidar_Radar_Mode)			//模式循环切换
				{
					Mode = Normal_Mode;
					Move_X = Move_Z = 0;
...
#if LD14P
						memset(Dataprocess,0, sizeof(LidarPointStructDef)*800);		//用于雷达检测障碍物的数组清零
#endif
				}
				break;
			default:break;
