...
//		else 								OLED_ShowString(0,50,"BTank");
	...
	else if(Mode == Smart_Cruise_Mode)
	{
		OLED_ShowString(0,0,"Mode:Smart");

		OLED_ShowString(0,10,"S:");
		OLED_ShowNumber(12,10,SmartCruise_Telemetry.state,1,12);
		OLED_ShowString(30,10,"F:");
		OLED_ShowNumber(42,10,SmartCruise_Sectors.front_mm,4,12);
		OLED_ShowString(78,10,"N:");
		OLED_ShowNumber(90,10,SmartCruise_Telemetry.nearest_mm,4,12);

		OLED_ShowString(0,20,"L:");
		OLED_ShowNumber(12,20,SmartCruise_Sectors.front_left_mm,4,12);
		OLED_ShowString(50,20,"R:");
		OLED_ShowNumber(62,20,SmartCruise_Sectors.front_right_mm,4,12);
		OLED_ShowString(98,20,"C:");
		OLED_ShowNumber(110,20,SmartCruise_Telemetry.obstacle_count,3,12);

		OLED_ShowString(00,30,"VX:");
		...
		if(MotorA.Motor_Pwm<0) {OLED_ShowString(14,40,"-"); OLED_ShowNumber(22,40,-MotorA.Motor_Pwm,4,12);}
		else {OLED_ShowString(14,40,"+"); OLED_ShowNumber(22,40,MotorA.Motor_Pwm,4,12);}
		OLED_ShowString(66,40,"E:");
		if(MotorB.Motor_Pwm<0) {OLED_ShowString(80,40,"-"); OLED_ShowNumber(88,40,-MotorB.Motor_Pwm,4,12);}
		else {OLED_ShowString(80,40,"+"); OLED_ShowNumber(88,40,MotorB.Motor_Pwm,4,12);}
	...
	else if(Mode == Lab_Lidar_Radar_Mode)
	{
		...
		OLED_ShowString(0,10,"S:");
		OLED_ShowNumber(12,10,Lab_Lidar_State,1,12);
		OLED_ShowString(30,10,"R:");
		OLED_ShowNumber(42,10,Lab_Lidar_Round,1,12);
		OLED_ShowString(58,10,"D:");
		OLED_ShowNumber(70,10,Lab_Lidar_Min_Distance,4,12);

		OLED_ShowString(0,20,"VX:");
		if(Move_X<0)	OLED_ShowString(24,20,"-");
		else			OLED_ShowString(24,20,"+");
		OLED_ShowNumber(32,20,myabs(Move_X*1000),3,12);
		OLED_ShowString(66,20,"VZ:");
		if(Move_Z<0)	OLED_ShowString(90,20,"-");
		else			OLED_ShowString(90,20,"+");
		OLED_ShowNumber(98,20,myabs(Move_Z*100),3,12);

		OLED_ShowString(00,30,"L-P:");
		if(MotorA.Motor_Pwm<0)	{OLED_ShowString(30,30,"-"); OLED_ShowNumber(36,30,-MotorA.Motor_Pwm,4,12);}
		else					{OLED_ShowString(30,30,"+"); OLED_ShowNumber(36,30, MotorA.Motor_Pwm,4,12);}
		OLED_ShowString(66,30,"L-E:");
		if(MotorA.Target_Encoder<0)	{OLED_ShowString(94,30,"-"); OLED_ShowNumber(104,30,-MotorA.Target_Encoder*1000,4,12);}
		else					{OLED_ShowString(94,30,"+"); OLED_ShowNumber(104,30, MotorA.Target_Encoder*1000,4,12);}

		OLED_ShowString(00,40,"R-P:");
		if(MotorB.Motor_Pwm<0)	{OLED_ShowString(30,40,"-"); OLED_ShowNumber(36,40,-MotorB.Motor_Pwm,4,12);}
		else					{OLED_ShowString(30,40,"+"); OLED_ShowNumber(36,40, MotorB.Motor_Pwm,4,12);}
		OLED_ShowString(66,40,"R-E:");
		if(MotorB.Target_Encoder<0)	{OLED_ShowString(94,40,"-"); OLED_ShowNumber(104,40,-MotorB.Target_Encoder*1000,4,12);}
		else					{OLED_ShowString(94,40,"+"); OLED_ShowNumber(104,40, MotorB.Target_Encoder*1000,4,12);}
...
//		else if(Mode==Measure_Distance_Mode)	OLED_ShowString(50,0,"Measure");//超声波测量距离
		...
		//====================第二行显示小车控制方式================//
			...
			else if(Remote_ON_Flag==RC_ON)	OLED_ShowString(0, 10, "Remote    ");
...
//										OLED_ShowNumber(84,10,Distance1,4,12);//显示超声波测试的距离
		...
		if(Car_Num == Akm_Car)//阿克曼车型转向速度显示角度
		{
			...
			if( Move_X<0)		OLED_ShowString(50,20,"-");
		...
		else//其他车型转向速度显示差速转向速度
		{
			...
			if( Move_X<0)		OLED_ShowString(50,20,"-");
									...
									OLED_ShowNumber(60,20,myabs(Move_X*1000),4,12);
			if( Move_Z<0)		OLED_ShowString(94,20,"-");
			else					OLED_ShowString(94,20,"+");
									OLED_ShowNumber(106,20,myabs(Move_Z*100),3,12);
...
{    
	...
	if(PID_Send==1)			//发送PID参数,在APP调参界面显示
	{
	   ...
	   printf("{A%d:%d:%d:%d:%d:%d:%d:%d:%d}$",(int)Voltage_Show,(int)SmartCruise_Telemetry.state,(int)SmartCruise_Telemetry.turn_dir,(int)SmartCruise_Sectors.front_mm,(int)SmartCruise_Sectors.front_left_mm,(int)SmartCruise_Sectors.front_right_mm,(int)SmartCruise_Sectors.left_mm,(int)SmartCruise_Sectors.right_mm,(int)SmartCruise_Telemetry.obstacle_count); //打印到APP上面
	 else				//发送小车姿态角，在波形界面显示
	    printf("{B%d:%d:%d:%d:%d:%d:%d}$",(int)MotorA.Current_Encoder,(int)MotorB.Current_Encoder,(int)Watch_Front_Hit_MM,(int)Watch_Front_Hit_Angle_X10,(int)Watch_Nearest_Body_Angle_X10,(int)(Move_X*1000),(int)(Move_Z*100)); //显示左右编码器的速度																	   //可按格式自行增加显示波形，最多可显示五个
}
...
{   
	...
	DataScope_Get_Channel_Data( MotorA.Target_Encoder*1000, 1 );     	//显示左轮速度，单位mm/s
	DataScope_Get_Channel_Data( MotorB.Target_Encoder*1000, 2 );    	//显示右轮速度
