...
/* HAL-compatible delay_ms using HAL_Delay */
    ...
    unsigned char *more)
{
    ...
    static unsigned char fifo_dbg_cnt = 0;

    if (!st.chip_cfg.dmp_on) {
        if (fifo_dbg_cnt < 3) { fifo_dbg_cnt++; log_e("[FIFO] dmp_on=0\n"); }
        return -1;
    }
    if (!st.chip_cfg.sensors) {
        if (fifo_dbg_cnt < 3) { fifo_dbg_cnt++; log_e("[FIFO] sensors=0\n"); }
        return -1;
    }
    ...
    if (fifo_count < length) {
        more[0] = 0;
        if (fifo_dbg_cnt < 3) { fifo_dbg_cnt++; log_e("[FIFO] empty: cnt=%u need=%u\n", fifo_count, length); }
        return -1;
    }
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        ...
        if (tmp[0] & BIT_FIFO_OVERFLOW) {
            if (fifo_dbg_cnt < 3) { fifo_dbg_cnt++; log_e("[FIFO] OVERFLOW, reset\n"); }
            mpu_reset_fifo();
            return -2;
        }
    }
