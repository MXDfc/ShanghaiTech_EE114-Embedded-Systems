...
/* HAL-compatible delay_ms and get_ms */
#define delay_ms    HAL_Delay

static inline void myget_ms(unsigned long *time)
{
    *time = HAL_GetTick();
}
    ...
    unsigned long *timestamp, short *sensors, unsigned char *more)
{
    ...
    if (dmp.feature_mask & (DMP_FEATURE_LP_QUAT | DMP_FEATURE_6X_LP_QUAT)) {
        ...
        if ((quat_mag_sq < QUAT_MAG_SQ_MIN) ||
            ...
            {
                static unsigned char qdbg_cnt = 0;
                if (qdbg_cnt < 5) {
                    qdbg_cnt++;
                    log_e("[QUAT_CHK] FAIL #%d: q14=(%ld,%ld,%ld,%ld) mag_sq=%ld range=[%ld,%ld]\n",
                        (int)qdbg_cnt,
                        quat_q14[0], quat_q14[1], quat_q14[2], quat_q14[3],
                        quat_mag_sq, (long)QUAT_MAG_SQ_MIN, (long)QUAT_MAG_SQ_MAX);
                    log_e("[QUAT_CHK] raw: q=(%ld,%ld,%ld,%ld) pktlen=%d fifo[0..3]=%02X %02X %02X %02X\n",
                        quat[0], quat[1], quat[2], quat[3],
                        (int)dmp.packet_length,
