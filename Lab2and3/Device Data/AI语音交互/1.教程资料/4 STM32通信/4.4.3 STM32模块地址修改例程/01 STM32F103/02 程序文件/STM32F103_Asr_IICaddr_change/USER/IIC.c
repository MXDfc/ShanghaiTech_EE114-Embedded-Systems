...
volatile int IIC_slave_Addr = Asr_Addr;
...
//设置模块IIC地址
void Asr_IICaddr_change(u8 newaddr)
{	
		IIC_Start(); //起始信号
		IIC_Send_Byte(IIC_slave_Addr<<1 | 0); //发送写指令
		IIC_Wait_Ack(); //应答
		IIC_Send_Byte(ASR_IIC_ADDR_CHANGE_ADDR); //发送修改IIC地址指令
		IIC_Wait_Ack(); //应答
		//IIC_Stop();

		IIC_Send_Byte(newaddr);
		IIC_Wait_Ack(); //应答
		IIC_slave_Addr = newaddr;
...
int Asr_Result()
{
	...
	IIC_Stop(); //停止	
	IIC_Start(); //起始信号
	IIC_Send_Byte(IIC_slave_Addr<<1 | 1); //发送读指令
	IIC_Wait_Ack(); //应答
