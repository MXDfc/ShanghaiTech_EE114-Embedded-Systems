...
//	RCC_APB2PeriphClockCmd(PWM_TIM1_CH1_GPIO_CLK,ENABLE);//航模遥控第四路接口
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 				//PA3 输入  
	GPIO_Init(PWM_TIM2_CH4_PORT, &GPIO_InitStructure);


	//第二路
	GPIO_InitStructure.GPIO_Pin  = PWM_TIM2_CH3_PIN; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 				//PA2 输入  
...
//	TIM_ICInitStructure.TIM_Channel = TIM_Channel_1; 			//通道4
//	TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;	//上升沿捕获
//	TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_DirectTI;
//	TIM_ICInitStructure.TIM_ICPrescaler = TIM_ICPSC_DIV1;	 	//配置输入分频,不分频 
//	TIM_ICInitStructure.TIM_ICFilter = 0x00;					//配置输入滤波器 不滤波
//	TIM_ICInit(PWM_TIM1, &TIM_ICInitStructure);
	...
	//使能中断设置
	//TIM2_CH3,CH4
	TIM_ITConfig(PWM_TIM2,TIM_IT_CC3|TIM_IT_CC4,ENABLE);	//允许捕获中断	
	//TIM1_CH1，CH4
//	TIM_ITConfig(PWM_TIM1,TIM_IT_Update|TIM_IT_CC1|TIM_IT_CC4,ENABLE);	//允许更新中断 ,允许CC4IE捕获中断	
...
//				Distance_TIM2_CH2_ICUserValueStructure.Capture_CcrValue = 0;
			...
			{
				// 获取捕获比较寄存器的值，这个值就是捕获到的高电平的时间的值
				Remoter_Ch2 = TIM_GetCapture3 (PWM_TIM2)-PWM_TIM2_CH3_ICUserValueStructure.Capture_CcrValue;
...
//				Distance_TIM2_CH2_ICUserValueStructure.Capture_CcrValue = 0;
			...
			{
				// 获取捕获比较寄存器的值，这个值就是捕获到的高电平的时间的值
				Remoter_Ch1 = TIM_GetCapture4(PWM_TIM2)-PWM_TIM2_CH4_ICUserValueStructure.Capture_CcrValue;
...
//	}
//	TIM_ClearITPendingBit (CAPTURE_TIM2,TIM_IT_CC3|TIM_IT_CC4);	
...
//{
//	static u8 ch3_filter_times=0, ch4_filter_times=0;
//		/*************************************通道1*******************************************/
...
//			{
//				// 获取捕获比较寄存器的值，这个值就是捕获到的高电平的时间的值
//				Remoter_Ch4 = TIM_GetCapture1(PWM_TIM1)-PWM_TIM1_CH1_ICUserValueStructure.Capture_CcrValue;
